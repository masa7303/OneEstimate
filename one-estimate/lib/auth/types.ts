...
export type AuthClient = {
  ...
  isLoading: boolean
  error?: string | null
  signIn: (opts: { email: string; password: string }) => Promise<{ ok: boolean; error?: string }>
  signUp: (opts: { email: string; password: string; verificationCallbackUrl?: string }) => Promise<{ ok: boolean; info?: string; error?: string }>

...
function getClient() {
  ...
  if (!url || !anon) {
    throw new Error("Supabase環境変数が未設定です。NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY を設定してください。")
  }
...
export function useAuth(): AuthClient {
  ...
  return useMemo(() => ({
    ...
    async sendPasswordReset(email: string) {
      const origin = typeof window !== 'undefined' ? window.location.origin : ''
      const { error } = await getClient().auth.resetPasswordForEmail(email, origin ? { redirectTo: `${origin}/auth/password-reset` } : undefined as any)
      if (error) return { ok: false, error: error.message }
      return { ok: true }
    },
    async verifyEmail(_code: string) {
      // Supabaseはメール内リンクで完了するため、ここでは何もしない
    ...
    async resendEmailVerification() {
      ...
      // supabase-js v2: https://supabase.com/docs/reference/javascript/auth-resend
      const { error } = await getClient().auth.resend({ type: 'signup', email: emailAddr })
      if (error) return { ok: false, error: error.message }
      return { ok: true }
    }

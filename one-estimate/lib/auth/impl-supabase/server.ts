...
export async function deleteUser(userId: string): Promise<{ ok: boolean; error?: string; status?: number }> {
  ...
  if (!url || !service) return { ok: false, error: 'Supabaseの設定が未完了です (NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY)', status: 503 }
  try {
    ...
    return { ok: false, error: e?.message || 'Supabaseユーザー削除に失敗しました', status: 502 }
  }
}

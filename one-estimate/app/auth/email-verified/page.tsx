...
export default function EmailVerifiedPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const r = await fetch(`/api/auth/neon/verify?code=${encodeURIComponent(code)}`, { cache: 'no-store' })
        ...
        if (r.ok && res && res.ok) {
          ...
          setTimeout(() => { try { window.location.assign('/dashboard') } catch { router.replace('/dashboard') } }, 800)
        } else {
          setStatus('error')
          setMessage(res?.error || '検証に失敗しました。リンクの有効期限が切れている可能性があります。')
        }
      } catch (e) {
        setStatus('error')

...
export default function OptionItemsPage() {
  ...
  const handleDelete = async (itemId: string, name: string) => {
    ...
    await fetch(`/api/admin/options/items/${itemId}`, { method: 'DELETE' })
    fetchItems()
  }
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/options" className="text-gray-400 hover:text-gray-600">← 戻る</Link>
        <h1 className="text-2xl font-bold text-gray-900">{category?.name} のアイテム</h1>
      </div>

      {/* 新規追加ボタン / フォーム */}
      {!showForm ? (
        <div className="mb-4">
          <Button onClick={() => setShowForm(true)}>アイテム追加</Button>
        </div>
      ) : (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={formName} onChange={e => setFormName(e.target.value)} required
                ...
                placeholder="例: IHクッキングヒーター" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">説明</label>
              <input type="text" value={formDesc} onChange={e => setFormDesc(e.target.value)}
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="例: 3口IH 標準グレード" />
            </div>
            <div>
